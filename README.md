...
- **Strict `@mention` routing** — `send_message` rejects unmatched handles up-front and returns the active worker list so the caller can self-correct. Matching is case-insensitive (`@Alice` finds worker `alice`).
...
| `send_message` | `team`, `text` | — | Send as the lead. `text` MUST contain `@handles`, and ALL of them must match active workers; unmatched handles fail the call with the active worker list. Mixed live/dead recipient lists return `dead_recipients_hint` plus `[SYSTEM]` notices delivered to the lead's inbox. |
