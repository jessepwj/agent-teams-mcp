...
fn read_codex_conversation(
    ...
    if exact_match {
        limitations.push(
            "Matched by the codex thread id persisted from the backend session."
                .into(),
        );
    } else {
        limitations.push(
    ...
    match kind {
        ...
        "reasoning" => {
            ...
            let text = summary_text
                .unwrap_or_else(|| "[reasoning, content not stored on disk]".into());
        ...
        "function_call_output" => {
            ...
            items.push(conversation_tool_item(
                ...
                None,
                None,
                parsed_output.or_else(|| raw_output.map(|s| Value::String(s.into()))),
                is_error,
                timestamp,
            ));
        }
