...
impl TeamModeToolset {
    pub(super) fn worker_add(&self, args: &Map<String, Value>) -> Result<ToolExecution> {
        ...
        // BUG-8 root cause: the prior default system_prompt was literally
        // `"You are alice"`. Codex saw that thin framing plus a tool list
        // and defaulted to chat-style text output — the lead saw nothing
        // because plain text never traverses the send_message route.
        // Fix: keep the caller's system_prompt (or the simple default)
        // intact as the primary role framing, then APPEND a single
        // protocol reminder at the end so it reads last in the prompt
        // (LLMs weight the most recent instruction heavily). This avoids
        // reshaping user-supplied prompts — a custom "You are a Rust
        // reviewer focused on X" still leads, and only one reply-routing
        // sentence is added.
        let base = execution
            .system_prompt
            .clone()
