...
- **Hook delivery invariants（ADR-022）** —— worker reply 自动注入依赖 async hook、per-team pending file、原子 drain 三件事同时成立。**Why**：旧 sync shepherd loop 会阻塞 CC turn，PowerShell ancestor walk 期间可被 CC SIGKILL；单个全局 `lead_pending.jsonl` 迫使 hook 每次 fire 都做慢 routing；read+truncate 在多个 async-wake hook 并发时会双注入。**How to apply**：
  - `Stop` hook 必须 `asyncRewake: true`，timeout 单位是秒；hook 是 long-poll loop，命中 pending 时 stderr + exit 2 唤醒 CC；空闲时按 ADR-031 的 lock + sweep 维持 per-CC singleton。
...
- **async-wake Stop hook 必须 per-CC singleton（ADR-031）** —— hook 是 long-poll loop（ADR-022 的 asyncRewake long-poll 设计），但 mid-turn `PostToolUse` hook 几乎总是先于 turn end 抢 drain 掉 pending，导致 async-wake 永远等不到 `exit(2)`、每次 turn end 都累积一个 sleeping hook。**Why**：实测一个 CC 在 12 分钟里能积压 7 个 hook 进程，每个 11MB；原 ADR-022 注释假设 "前 hook 已 exit 才会有新 Stop event" 在用户 idle 时不成立。**How to apply**：(1) async-wake hook startup 必须写 `<project_root>/.agent-teams/runtime/stop-hook-<owner_cc_pid>.lock`（atomic rename）+ 执行 sweep 干掉同 owner_cc_pid 的其他 async-wake 进程。(2) 主 loop 每次 sleep 醒来读 lock，PID 不是自己 → graceful `exit(0)`、不删 lock。(3) 失败开放：lock 文件不存在 / 不可读 / 非数字 → 视为 "自己仍持有" 以容忍 transient I/O。(4) `cc_pid` 解析失败时跳过 lock + sweep，回退到原 long-poll 行为。(5) sweep 用 `proc.kill()`，目标限制在 `team_mode_service hook async-wake` 且 ancestor 同 owner_cc_pid，不能跨 CC。
